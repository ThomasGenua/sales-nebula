...
router.post('/bulk/status', authenticate, auditMiddleware, async (req, res, next) => {
  try {
    ...
    const updated = await Promise.all(ids.slice(0, 100).map(async (id) => {
      try { return await prisma.$executeRaw`UPDATE "leads" SET status = ${status} WHERE id = ${id}`; }
      catch (e) { return null; }
    }));
    await req.audit({ action: 'bulk_update', module: 'leads', details: `Bulk status update: ${ids.length} records to ${status}` });
